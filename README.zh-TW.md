...
「比較好」不代表某個遊戲的單一分數最高，而是它符合事先宣告的目的與硬性限制，在重要評估面向都有良好表現，並且對自己的設計主張擁有最有力的證據。

可以依照以下順序評估：

1. **先鎖定 brief 與硬性關卡。** 在看到候選方案分數以前，先定義目標對象、學習或娛樂目標、遊戲時間、無障礙需求、平台與不可妥協的限制。
2. **產生真正不同的候選方案。** 先產生 6–10 個在核心動詞、選擇結構、壓力模型、失敗模型或重玩結構上不同的概念。只有換皮或換顏色不算不同方案。
3. **執行盲評。** 分別從遊戲設計、第一次遊玩、領域正確性、研究效度與製作可行性檢查。不要讓作者身分或簡報風格決定結果。
4. **先套用硬性關卡。** 只要不符合不可妥協的學習、安全、無障礙、研究或製作條件，就淘汰該方案。平均分數高不能挽救硬性關卡失敗。
5. **分開評估不同面向。** `G` 是 Game Experience（遊戲體驗），`R` 是 Research Validity（研究效度），`D` 是 Development Feasibility（開發可行性）。如果是非研究型娛樂遊戲，可以把 `R` 換成 Purpose Fit（目的適配度）。不要把所有弱點藏在一個平均分數裡。
6. **加入教育證據。** 教育遊戲要另外計算 `R_edu`，不要把它直接併入 `G`、`R` 或 `D`。它把 Learning（學習，`L`）、Engagement（投入，`E`）、Motivation/self-efficacy（動機／自我效能，`M`）與 Appropriate challenge（適切挑戰，`A`）加權，並扣除 Guessing/exploitation（猜測／鑽漏洞，`G`）與 Cognitive overload（認知超載，`C`）：

   `R_edu = wL*L + wE*E + wM*M + wA*A - pG*G - pC*C`

   `R_edu` 是設計診斷，不是學習成效的證明，也不能取代正式評量。它的作用是揭露「很好玩但沒有對準學習」或「學習內容正確但負荷過高」等衝突。
7. **測試風險最高的假設。** 建立最小的垂直切片，實際檢查核心選擇、回饋、學習任務或製作風險是否成立。
8. **由人類做最後決定。** 比較通過關卡的方案與剩餘不確定性。如果沒有單一方案在所有面向都勝出，就使用 Pareto front（帕累托前緣）並公開 tie-breaker（平手決勝規則）。最後由教師、研究者或專案負責人決定核准、修改或暫停。

證據應標記為 `hypothesis`（假設）、`ai_pre_score`（AI 預評）、`expert_review`（專家審查）、`prototype_observation`（原型觀察）或 `player_evidence`（玩家證據）。只有 AI 產生的分數不能被視為高信心驗證。
